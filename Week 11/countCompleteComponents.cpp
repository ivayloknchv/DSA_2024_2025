...
public:
    ...
    void dfs(unordered_map<int, unordered_set<int>>& graph, unordered_set<int>& visited,
    int& nodesCount, int&edgesCount , int curr)
    {
        visited.insert(curr);
        nodesCount++;
        ...
        for(int neighbour : graph[curr])
        {
            if(visited.count(neighbour) == 0)
            {
                dfs(graph, visited, nodesCount, edgesCount, neighbour);
            }
        }
    }
    int countCompleteComponents(int n, vector<vector<int>>& edges) 
    {
        ...
        for(int i = 0; i < n; i++)
        {
            ...
            if(!visited.count(i))
            {
                ...
                dfs(graph, visited, nodesCount, edgesCount, i);
                edgesCount/=2;
                if(nodesCount*(nodesCount-1)/2 == edgesCount)
                {
                    ans++;
                }
